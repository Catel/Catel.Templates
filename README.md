...
#### WPF Application with Catel

A .NET 8.0 project template for creating a WPF application using the Catel framework.

Template name: `WPF Application with Catel`

#### WPF Application with Catel and Orchestra (Fluent Ribbon)

A project template for creating a WPF application using the Catel framework and the Fluent.Ribbon shell from Orchestra.

Template name: `WPF Application using Catel and the Fluent.Ribbon shell from Orchestra.`

### Item Templates

#### ViewModel (Catel)

A template for a new view model from the Catel framework.

Template name: `ViewModel (Catel)`

#### DataWindow (WPF with Catel)

A template to create a Catel DataWindow that is ready to be used with a view model.

Template name: `DataWindow (WPF with Catel)`

#### User Control (WPF with Catel)

An item template for a user control created with the Catel framework.

Template name: `User Control (WPF with Catel)`
...
1. Copy the templates from the repository to VS 2022

Use the `scripts - Copy templates to VS2022.ps1` script or manually copy the templates from `/templates/` to `%documents%\Visual Studio 2022\`.
...
To customize the templates, edit the files in `%documents%\Visual Studio 2022\Templates`.

Once customized, you can directly test them in Visual Studio.

3. Copy the changes back

To copy the updated templates back to the solution, use the script `scripts - Copy templates from VS2022.ps1` or manually copy the templates from `%documents%\Visual Studio 2022\` to `/templates/`.
